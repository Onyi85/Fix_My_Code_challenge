...
class User():
    ...
    def password(self, pwd):
        ...
        """
        if pwd is None or not isinstance(pwd, str):  # changed type() to isinstance()
            self.__password = None
        else:
            self.__password = hashlib.md5(pwd.encode()).hexdigest().lower()  # fixed typo (_password to __password)
    ...
    def is_valid_password(self, pwd):
	    ...
	    return False
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password  # changed upper() to lower()
...
if _name_ == '_main_':
    ...
    user_2.password = "Not a pwd"
    if user_2.password is not None:
        print("User.password should be None if set to a non-string")
